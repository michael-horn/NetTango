...
to go
  ...
  tick
  if mouse-down? [ handle-mouse-down mouse-xcor mouse-ycor ]
end
...
to handle-mouse-down [tx ty]
  ;; get the spawner closest to where the user clicked
  let new-parent min-one-of spawners [distancexy tx ty]
  ifelse asexual?
...
to touch-down [touch-x touch-y]
   handle-mouse-down touch-x touch-y
end
